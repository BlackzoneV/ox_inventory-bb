...
function Inventory.AddItem(inv, item, count, metadata, slot, cb)
	...
	if not Inventory.CanCarryItem(inv.id, item.name, count) then
		return
	end
	
	if item.name == "tape" then
		metadata = metadata or {}
		metadata.uses = metadata?.uses or 3
		metadata.description = "Kullanım Hakkı: "..metadata?.uses or 3
	end
