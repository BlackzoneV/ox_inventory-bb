			...
			if currentWeapon and currentWeapon.timer then
				...
				if not invBusy and currentWeapon.timer ~= 0 and currentWeapon.timer < GetGameTimer() then
					...
					if weaponAmmo then
						...
						local _w, inClip = GetAmmoInClip(playerPed, currentWeapon.hash)

						if client.autoreload and currentWeapon.ammo and inClip == 0 then
							if cache.vehicle then
								TaskReloadWeapon(playerPed, false)
							else
...
RegisterNUICallback('giveItem', function(data, cb)
	...
	if client.giveplayerlist then
		...
		if nearbyCount == 0 then return lib.notify({ type = 'error', description = 'Yakınında hiç kimse yok!' }) end
		...
		for i = 1, #nearbyPlayers do
            ...
            if isGiveTargetValid(option.ped, option.coords) then
				...
				option.title = ('[%s]'):format(option.id)
		...
		lib.registerContext({
		  ...
		  title = 'Eşya Ver',
		  onBack = function()
		    print('Eşya vermekten vazgeçildi.')
		  end,
		  options = giveList
		})
